...
import { something } from './packets.js'
...
ipcMain.on('output', (event, value) => {
  ...
  board.on('ready', () => {
    ...
    mainWindow.webContents.send('update-html-text', ['Connected to Arduino', true])
    let pitch, yaw, roll
    ...
    ipcMain.on('data-to-main', (event, data) => {
      ...
      if (gForceMode) {
        ...
        a.to(Math.max(minAngle, Math.min(maxAngle, maxAngle - 60 + pitchVal - rollVal / 1.4)))
        b.to(Math.max(minAngle, Math.min(maxAngle, maxAngle - 60 - rollVal)))
        c.to(Math.max(minAngle, Math.min(maxAngle, maxAngle - 60 - pitchVal - rollVal / 1.4)))
        d.to(Math.max(minAngle, Math.min(maxAngle, maxAngle - 60 - pitchVal + rollVal / 1.4)))
        e.to(Math.max(minAngle, Math.min(maxAngle, maxAngle - 60 + rollVal)))
        f.to(Math.max(minAngle, Math.min(maxAngle, maxAngle - 60 + pitchVal + rollVal / 1.4)))
  ...
  // update html text
  board.on('exit', () => {
    mainWindow.webContents.send('update-html-text', ['Disconnected from Arduino', true])
  })
})
