...
ipcRenderer.on('data', (event, data) => {
  ...
  document.getElementById('pitchText').textContent = `Pitch: ${data[3].toFixed(1)}`
  document.getElementById('yawText').textContent = `Yaw: ${data[2].toFixed(1)}`
  document.getElementById('rollText').textContent = `Roll: ${data[4].toFixed(1)}`
  document.getElementById('xAccelText').textContent = `xAccel: ${(data[5] * 10).toFixed(1)}`
  document.getElementById('yAccelText').textContent = `yAccel: ${(data[6] * 10).toFixed(1)}`
  document.getElementById('zAccelText').textContent = `zAccel: ${(data[10] * 10).toFixed(1)}`
  document.getElementById('heaveText').textContent = `Heave: ${(data[10] * 10).toFixed(1)}`
  document.getElementById('swayText').textContent = `Sway: ${(data[5] * 10).toFixed(1)}`
  document.getElementById('surgeText').textContent = `Surge: ${(data[6] * 10).toFixed(1)}`
...
ipcRenderer.on('update-html-text', (event, newText) => {
  ...
  if (newText[1] == true) document.getElementById('arduinoConnected').style.color = 'green'
...
function changeConnectionStatus(data) {
  if (data >= 0) {
    ...
    timeout = setTimeout(() => {
      document.getElementById('connectionStatus').innerHTML = 'Not connected to a game'
